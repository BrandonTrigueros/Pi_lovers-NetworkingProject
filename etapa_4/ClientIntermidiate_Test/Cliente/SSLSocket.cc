...
SSLSocket::SSLSocket(int id) { this->InitVSocket(id); }
 ...
 *
 *  Creates a new SSL server context to start encrypted comunications, this
 *  context is stored in class instance
 *
 **/
void SSLSocket::InitContext(bool serverContext) {
  ...
  if (nullptr == context) {
    throw std::runtime_error(
        "SSLSocket::InitContext( bool )" + std::to_string(ERR_get_error()));
  }
...
int SSLSocket::Connect(const char* hostName, int port) {
  ...
  if (0 == st) {
    ...
    throw std::runtime_error(
        "SSLSocket::Connect( const char *, int )" + std::to_string(st));
  } else if (st < 0) {
    throw std::runtime_error(
        "SSLSocket::Connect( const char *, int ) - Fatal error");
  }
  return st;
}
